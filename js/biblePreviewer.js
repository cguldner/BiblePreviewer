...
import {DASHES_STR, getVerseFromString, splitReferenceDisplayText, splitVerseList} from './verseParser.mjs';
...
if (GENERATE_REGEX) {
    ...
    bibleRegex += `\\.?\\s*(\\d{1,3}:\\s*\\d{1,3}(?:[,;:${DASHES_STR}]\\s*\\d{1,3}|\\s+and\\s+\\d{1,3}`;
    ...
    for (const node of nodeList) {
        ...
        renderNode.innerHTML = node.textContent.replace(bibleRegex, function (orig, matchedBook, verseListString, judeVerse) {
            ...
            if (judeVerse === undefined) {
                ...
                book = JUDE_BOOK_ID;
                verseListString = splitVerseList(judeVerse);
            ...
            let referenceList = [];
            const verseList = splitVerseList(verseListString);
            const splitText = splitReferenceDisplayText(orig);
